...
class TextGenerator:
    def __init__(self,
        ...
        self.ctx = nullcontext() if self.device_type == 'cpu' else torch.amp.autocast(device_type=self.device_type,
                                                                                      dtype=ptdtype)
    ...
    def chat_template(self, prompt):
        ...
        # 构建 Llama-2 格式的对话
        # 注意：这里必须包含 System Prompt，且格式严格
        return f"{B_INST} {B_SYS}{DEFAULT_SYSTEM_PROMPT}{E_SYS}{prompt.strip()} {E_INST}"
        ...
        with torch.no_grad():  # 禁用梯度计算，提升效率
            with self.ctx:  # 进入自动混合精度的上下文（如果是 GPU 并使用 float16 时）
                for k in range(num_samples):  # 循环生成指定数量的样本
                    y = self.model.generate(x, self.tokenizer.eos_token_id, max_new_tokens, temperature=temperature,
                                            top_k=top_k)  # 生成文本
                    # 只获取新生成的 Token (去除输入的 Prompt 部分)
                    new_tokens = y[0].tolist()[len(start_ids):]
                    generated_texts.append(self.tokenizer.decode(new_tokens, skip_special_tokens=True))
...
if __name__ == "__main__":
    ...
    for i in range(len(pretrain_prompt_datas)):
        samples = generator.pretrain_sample(start=pretrain_prompt_datas[i], num_samples=1, max_new_tokens=120,
                                            temperature=0.75)
