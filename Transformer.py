...
class Transformer(PreTrainedModel):
    """
    Transformer模型，继承自PreTrainedModel。
    """
    ...
    def forward(self, tokens: torch.Tensor, targets: Optional[torch.Tensor] = None, **kwargs) -> torch.Tensor:
        ...
        前向传播函数。
        Args:
            tokens (torch.Tensor): 输入的 token 序列，形状为 [batch_size, seq_len]。
            targets (torch.Tensor, optional): 目标序列，形状为 [batch_size, seq_len]。
        Returns:
            torch.Tensor: 预测的 logits 序列，形状为 [batch_size, seq_len, vocab_size]。
        """
